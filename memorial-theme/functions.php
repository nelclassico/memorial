...
function renderizar_metabox_galeria_simples($post) {
    ...
    echo '<input type="text" name="titulo" value="' . esc_attr(get_post_meta($post_id, 'titulo', true)) . '" class="large-text" />';

    // Adicione o campo de seleção para o número de colunas
    echo '<p>Número de colunas:</p>';
    echo '<select name="num_colunas">';
    echo '<option value="col-md-2" ' . (esc_attr(get_post_meta($post_id, 'num_colunas', true)) === 'col-md-2' ? 'selected' : '') . '>5 colunas</option>';
    echo '<option value="col-md-3" ' . (esc_attr(get_post_meta($post_id, 'num_colunas', true)) === 'col-md-3' ? 'selected' : '') . '>4 colunas</option>';
    echo '<option value="col-md-4" ' . (esc_attr(get_post_meta($post_id, 'num_colunas', true)) === 'col-md-4' ? 'selected' : '') . '>3 colunas</option>';
    echo '<option value="col-md-6" ' . (esc_attr(get_post_meta($post_id, 'num_colunas', true)) === 'col-md-6' ? 'selected' : '') . '>2 colunas</option>';
    ...
    <div id="conteudos-container-galeria">
        <?php foreach ($conteudos as $index => $conteudo): ?>
            <div class="conteudo" style="margin-bottom: 10px;">
                ...
                <input type="text" name="conteudos-galeria[<?php echo $index; ?>][imagem]" value="<?php echo esc_attr($conteudo['imagem']); ?>" />
                <button class="upload-imagem-button" id="upload-imagem-button-<?php echo $index; ?>">Upload de Imagem</button><br> <!-- Botão para fazer o upload da imagem -->
            ...
            $('#adicionar-conteudo').on('click', function() {
                ...
                var novoConteudo = `
                    <div class="conteudo" style="margin-bottom: 10px;">
                        ...
                        <input type="text" name="conteudos-galeria[${index}][imagem]" id="imagem_${index}" />
                        <button class="upload-imagem-button" id="upload-imagem-button-${index}">Upload de Imagem</button><br> <!-- Botão para fazer o upload da imagem -->
                ...
                // Botão para fazer o upload de imagem para conteúdo específico
                $('#upload-imagem-button-' + index).click(function() {
                    var send_attachment_bkp = wp.media.editor.send.attachment;
                    wp.media.editor.send.attachment = function(props, attachment) {
                        $('[name="conteudos-galeria[' + index + '][imagem]').val(attachment.url);
...
function galeria_simples_shortcode($atts) {
    ...
    if ($conteudos && is_array($conteudos)) {
        foreach ($conteudos as $conteudo) {
            ...
            $title = isset($conteudo['titulo']) ? $conteudo['titulo'] : '';
            $content = isset($conteudo['conteudo']) ? wp_kses($conteudo['conteudo']) : '';
